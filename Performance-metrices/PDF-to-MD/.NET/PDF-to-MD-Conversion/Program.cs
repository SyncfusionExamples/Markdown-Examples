...
class Program
{
    static void Main()
    {
		...
		using (FileStream stream = new FileStream(Path.GetFullPath("Data/Document-50.pdf"), FileMode.Open, FileAccess.Read))
		{
			...
			File.WriteAllText(Path.GetFullPath("Output/Result.md"), data, Encoding.UTF8);
		}
		stopwatch.Stop();
